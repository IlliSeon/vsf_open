...
vsfusbd_evt_handler(struct vsfsm_t *sm, vsfsm_evt_t evt)
{
	...
	case VSFSM_EVT_INIT:
		{
			...
			if (drv->callback != NULL)
			{
				...
				drv->callback->on_event = vsfusbd_on_EVENT;
			}
	...
	case VSFUSBD_INTEVT_SOF:
		{
			uint32_t frame_no = drv->get_frame_number();
