...
vsf_err_t vsf_dynarr_set_size(struct vsf_dynarr_t *dynarr, uint32_t size)
{
	...
	struct vsf_dynarr_table_t *table = vsflist_get_container(dynarr->table.next,
	...
	if (buffer_num > buffer_num_orig)
	{
		...
		else
		{
			...
			while (table_idx--)
				table = vsflist_get_container(table->list.next,
					struct vsf_dynarr_table_t, list);
		}
		...
		while (buffer_num > 0)
		{
			...
			{
				if (!table)
					vsflist_insert(dynarr->table, new_table->list);
				else
					vsflist_insert(table->list, new_table->list);
				table = new_table;
			}
	...
	else if (buffer_num < buffer_num_orig)
	{
		if (!buffer_num)
		{
			...
			table = vsflist_get_container(&dynarr->table,
				struct vsf_dynarr_table_t, list);
		}
		else
		{
		...
		while (buffer_num > 0)
		{
			if (++buffer_idx == (1UL << dynarr->table_size_bitlen))
			{
				new_table = vsflist_get_container(table->list.next,
					struct vsf_dynarr_table_t, list);
				...
				table = vsflist_get_container(new_table->list.next,
					struct vsf_dynarr_table_t, list);
				vsf_bufmgr_free(new_table);
			}
			else
...
void *vsf_dynarr_get(struct vsf_dynarr_t *dynarr, uint32_t pos)
{
	if (pos < dynarr->length)
	{
		struct vsf_dynarr_table_t *table =
			vsflist_get_container(dynarr->table.next, struct vsf_dynarr_table_t,
				list);
...
vsf_err_t vsf_dynarr_init(struct vsf_dynarr_t *dynarr)
{
	...
	vsflist_init_node(dynarr->table);
	dynarr->length = 0;
	return VSFERR_NONE;
}
...
vsf_err_t vsf_dynarr_fini(struct vsf_dynarr_t *dynarr)
{
	...
	table = vsflist_get_container(dynarr->table.next, struct vsf_dynarr_table_t,
		list);
	while (buffer_num > 0)
	{
		tmp_table = table;
		table = vsflist_get_container(table->list.next, struct vsf_dynarr_table_t, list);
