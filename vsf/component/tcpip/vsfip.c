...
void vsfip_buffer_release(struct vsfip_buffer_t *buf)
{
	if ((buf != NULL) && buf->ref)
	{
		if (!--buf->ref)
		{
			if (buf->netif != NULL)
			{
				vsfdbg_printf("vsfip: free buffer 0x%04X with netif %04X ref %d\r\n", buf, buf->netif, buf->netif->ref);
				vsfip_netif_release(buf->netif);
				buf->netif = NULL;
			}
...
{
	buf->netif = netif;
	vsfdbg_printf("vsfip: set buffer 0x%04X with netif %04X ref %d\r\n", buf, buf->netif, buf->netif->ref);
	vsfip_netif_reference(netif);
}
							...
							struct vsfip_netif_t *netif)
{
	...
	// add to netif list
	netif->list.next = vsfip.netif_list.next;
	vsfip.netif_list.next = &netif->list;
							...
							struct vsfip_netif_t *netif)
{
	...
	if (!vsflist_remove(&vsfip.netif_list.next, &netif->list))
	{
		vsf_err_t err;
		...
		netif->init_pt.state = 0;
		vsfsm_pt_entry(pt);
		err = vsfip_netif_fini(&netif->init_pt, evt);
		if (err > 0) return err;

		vsfip_netif_release(netif);
		return err;
	}
...
static struct vsfip_netif_t* vsfip_ip_route(struct vsfip_ipaddr_t *addr)
{
...
struct vsfsm_state_t* vsfip_tick(struct vsfsm_t *sm, vsfsm_evt_t evt)
{
	...
	{
	case VSFSM_EVT_INIT:
		vsfsm_notifier_set_evt(&vsfip.tick_timer.notifier, sm, VSFIP_EVT_TICK);
	...
	case VSFIP_EVT_TICK:
		...
		vsflist_foreach(socket, vsfip.udpconns.next, struct vsfip_socket_t, list)
		{
			vsfip_check_buff(socket);
		}
		vsflist_foreach(socket, vsfip.tcpconns.next, struct vsfip_socket_t, list)
		{
			vsflist_foreach(child, socket->listener.child.next, struct vsfip_socket_t, list)
			{
...
{
	return vsfip_netif_ip_output(buf);
}
		...
		struct vsfip_buffer_t *buf)
{
	return (AF_INET == socket->family) ?
				vsfip_ip4_output(socket, buf) : vsfip_ip6_output(socket, buf);
}
		...
		struct vsfip_buffer_t *ipbuf)
{
	...
	if (ipbuf->app.buffer - ipbuf->buffer <
			sizeof(struct vsfip_ip4head_t))
	{
		return VSFERR_FAIL;
	}

	ipbuf->buf.buffer -= sizeof(struct vsfip_ip4head_t);
	ipbuf->buf.size += sizeof(struct vsfip_ip4head_t);
	ipbuf->iphead.ip4head = (struct vsfip_ip4head_t *)ipbuf->buf.buffer;
	ip4head = ipbuf->iphead.ip4head;
	ip4head->ver_hl = 0x45;		// IPv4 and 5 dwrod header len
	ip4head->tos = 0;
	ip4head->len = SYS_TO_BE_U16(ipbuf->buf.size);
	...
	ip4head->ipsrc = ipbuf->netif->ipaddr.addr.s_addr;
		...
		struct vsfip_buffer_t *buf)
{
	...
	if (NULL == buf->netif)
	{
		...
		vsfip_buffer_set_netif(buf, vsfip_ip_route(&socket->remote_sockaddr.sin_addr));
		vsfsm_sched_unlock(origlevel);
	}
	if (NULL == buf->netif)
	{
	...
	if (buf->buf.size >
			(buf->netif->mtu - sizeof(struct vsfip_ip4head_t)))
	{
	...
	err = vsfip_add_ip4head(socket, buf);
	if (err) goto cleanup;

	return vsfip_ip_output_do(buf);
cleanup:
	vsfip_buffer_release(buf);
	return err;
}
			...
			struct vsfip_socket_t *father, struct vsfip_sockaddr_t *sockaddr)
{
	...
	if (new_socket != NULL)
	{
		...
		new_socket->list.next = father->listener.child.next;
		father->listener.child.next = &new_socket->list;
	}
	return new_socket;
}
...
static vsf_err_t vsfip_free_socket(struct vsfip_socket_t *socket)
{
	...
	vsflist_remove(&list->next, &socket->list);
	vsfsm_sched_unlock(origlevel);
...
vsf_err_t vsfip_close(struct vsfip_socket_t *socket)
{
	...
	if (socket->father != NULL)
	{
		...
		if (!vsflist_remove(&father->listener.child.next, &socket->list))
		{
...
vsf_err_t vsfip_bind(struct vsfip_socket_t *socket, uint16_t port)
{
	...
	{
	case IPPROTO_UDP:
		head = &vsfip.udpconns.next;
		break;
	case IPPROTO_TCP:
		head = &vsfip.tcpconns.next;
		break;
	}

	// check if port already binded
	vsflist_foreach(__socket, *head, struct vsfip_socket_t, list)
	{
		if (__socket->local_sockaddr.sin_port == port)
		{
			vsfsm_sched_unlock(origlevel);
			return VSFERR_FAIL;
		}
...
// proto common
// TODO: fix for IPv6
static void vsfip_proto_input(struct vsflist_t *list,
				struct vsfip_buffer_t *buf, struct vsfip_protoport_t *port)
{
	...
	{
		local_addr = &__socket->local_sockaddr;
		// if port match and
		...
		{
			if (__socket->listener.backlog == 0)
			{
				// normal socket
				remote_addr = &__socket->remote_sockaddr;
			...
			{
				// listener socket
				vsflist_foreach(child, __socket->listener.child.next, struct vsfip_socket_t, list)
				{
			...
			struct vsfip_sockaddr_t *sockaddr, struct vsfip_buffer_t *buf)
{
	...
	err = vsfip_ip_output(socket, buf);
			...
			struct vsfip_buffer_t **buf)
{
	...
	if (socket->rx_timeout_ms)
	{
		...
		vsfsm_notifier_set_evt(&socket->rx_timer.notifier, pt->sm,
...
vsfip_tcp_accept_try(struct vsfip_socket_t *socket)
{
	...
	vsflist_foreach(__socket, socket->listener.child.next, struct vsfip_socket_t, list)
	{
		pcb = (struct vsfip_tcppcb_t *)__socket->pcb.protopcb;
		if ((pcb->state == VSFIP_TCPSTAT_ESTABLISHED) && !__socket->accepted)
		{
			__socket->accepted = true;
			return __socket;
		}
	}
	return NULL;
