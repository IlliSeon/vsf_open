...
static void usrapp_flash_on_finish(void *param, vsf_err_t err)
{
	struct usrapp_t *app = (struct usrapp_t *)param;
	app->vsfvm.err = err;
	vsfsm_post_evt_pending(app->vsfvm.notifier_sm, VSFSM_EVT_USER);
}

static vsf_err_t usrapp_write_flash(struct vsfsm_pt_t *pt, vsfsm_evt_t evt,
		uint32_t addr, uint32_t offset, uint8_t *buff, uint32_t size)
{
	if (usrapp.vsfvm.compiling || usrapp.vsfvm.remainsize)
		return VSFERR_FAIL;

	vsfsm_pt_begin(pt);

	usrapp.vsfvm.curaddr = addr + offset;
	usrapp.vsfvm.curbuff = buff;
	usrapp.vsfvm.remainsize = size;
	usrapp.vsfvm.pagesize = vsfhal_flash_blocksize(0, usrapp.vsfvm.curaddr,
			usrapp.vsfvm.remainsize, VSFHAL_FLASH_OP_WRITE);
	usrapp.vsfvm.notifier_sm = pt->sm;

	vsfhal_flash_config_cb(0, 0, &usrapp, usrapp_flash_on_finish);
	while (usrapp.vsfvm.remainsize)
	{
		if (!(usrapp.vsfvm.curaddr % usrapp.vsfvm.pagesize))
		{
			if (vsfhal_flash_erase(0, usrapp.vsfvm.curaddr))
				goto fail;
			vsfsm_pt_wfe(pt, VSFSM_EVT_USER);
			if (usrapp.vsfvm.err) goto fail;
		}

		if (vsfhal_flash_write(0, usrapp.vsfvm.curaddr, usrapp.vsfvm.curbuff))
			goto fail;
		vsfsm_pt_wfe(pt, VSFSM_EVT_USER);
		if (usrapp.vsfvm.err) goto fail;

		usrapp.vsfvm.remainsize -= usrapp.vsfvm.pagesize;
		usrapp.vsfvm.curaddr += usrapp.vsfvm.pagesize;
		usrapp.vsfvm.curbuff += usrapp.vsfvm.pagesize;
	}
...
fail:
	usrapp.vsfvm.remainsize = 0;
	return VSFERR_FAIL;
}

static vsf_err_t usrapp_read_flash(struct vsfsm_pt_t *pt, vsfsm_evt_t evt,
		uint32_t addr, uint32_t offset, uint8_t *buff, uint32_t size)
{
	memcpy(buff, (void *)(addr + offset), size);
	return VSFERR_NONE;
}
...
static vsf_err_t usrapp_write_source(struct vsfsm_pt_t *pt, vsfsm_evt_t evt,
		...
		uint32_t pagesize)
{
	if ((offset + pagesize) > APPCFG_VM_SOURCE_SIZE)
		return VSFERR_FAIL;
	return usrapp_write_flash(pt, evt, APPCFG_VM_SOURCE_ADDR, offset, buff, pagesize);
}
		...
		uint32_t pagesize)
{
	if ((offset + pagesize) > APPCFG_VM_SOURCE_SIZE)
		return VSFERR_FAIL;
	return usrapp_read_flash(pt, evt, APPCFG_VM_SOURCE_ADDR, offset, buff, pagesize);
}
...
static vsf_err_t usrapp_write_bytecode(struct vsfsm_pt_t *pt, vsfsm_evt_t evt,
		...
		uint32_t pagesize)
{
	if ((offset + pagesize) > APPCFG_VM_BYTECODE_SIZE)
		return VSFERR_FAIL;
	return usrapp_write_flash(pt, evt, APPCFG_VM_BYTECODE_ADDR, offset, buff, pagesize);
}
		...
		uint32_t pagesize)
{
	if ((offset + pagesize) > APPCFG_VM_BYTECODE_SIZE)
		return VSFERR_FAIL;
	return usrapp_read_flash(pt, evt, APPCFG_VM_BYTECODE_ADDR, offset, buff, pagesize);
}
...
static void usrapp_flash_on_erase_finish(void *param, vsf_err_t err)
{
	struct usrapp_t *app = (struct usrapp_t *)param;

	if (!err)
	{
		app->vsfvm.remainsize -= app->vsfvm.pagesize;
		app->vsfvm.curaddr += app->vsfvm.pagesize;

		if (!app->vsfvm.remainsize)
			vsfsm_post_evt_pending(&app->vsfvm.sm, USRAPP_EVT_VM_COMPILE);
		else
			vsfhal_flash_erase(0, usrapp.vsfvm.curaddr);
	}
	else
	{
		app->vsfvm.remainsize = 0;
	}
}
...
static vsf_err_t usrapp_write_compile(struct vsfsm_pt_t *pt, vsfsm_evt_t evt,
		...
		uint32_t pagesize)
{
	if (usrapp.vsfvm.remainsize)
		return VSFERR_FAIL;

	vsfhal_flash_config_cb(0, 0, &usrapp, usrapp_flash_on_erase_finish);
	usrapp.vsfvm.curaddr = APPCFG_VM_BYTECODE_ADDR;
	usrapp.vsfvm.remainsize = APPCFG_VM_BYTECODE_SIZE;
	usrapp.vsfvm.pagesize = vsfhal_flash_blocksize(0, usrapp.vsfvm.curaddr,
			usrapp.vsfvm.remainsize, VSFHAL_FLASH_OP_ERASE);
	return vsfhal_flash_erase(0, usrapp.vsfvm.curaddr);
}
...
{
	// block write
	vsfhal_flash_config_cb(0, 0, NULL, NULL);
	// vsfhal_flash_write MUST write 4bytes at one time
	if (vsfhal_flash_write(0, APPCFG_VM_BYTECODE_ADDR + pos << 2, (uint8_t *)&code))
		return -1;
	return 0;
}
...
{
	return pos < (APPCFG_VM_BYTECODE_SIZE >> 2) ?
		((uint32_t *)APPCFG_VM_BYTECODE_ADDR)[pos] : 0xFFFFFFFF;
}
...
usrapp_vm_evthandler(struct vsfsm_t *sm, vsfsm_evt_t evt)
{
...

	{
		...
		err = vsfvmc_input(vmc, (char *)APPCFG_VM_SOURCE_ADDR);
		if (err < 0)
		{
		err_return:
			err = -err;
		...
		if (!err)
		{
			...
			vsfvm_objdump((uint32_t *)APPCFG_VM_BYTECODE_ADDR, app->vsfvm.token_num);
		}
		goto compile_end;
	}
	case USRAPP_EVT_VM_RUN:
		if (!app->vsfvm.polling)
		{
			...
			script->token = (const uint32_t *)APPCFG_VM_BYTECODE_ADDR;
...
//	.usbh.hcd								= &vsfshci_drv,
	...
	.fakefat32.root_dir						=
	{
		...
		},
		{
			...
			.memfile.file.size				= APPCFG_VM_BYTECODE_SIZE,
			.memfile.file.attr				= VSFILE_ATTR_ARCHIVE | VSFILE_ATTR_HIDDEN | VSFILE_ATTR_SYSTEM,
